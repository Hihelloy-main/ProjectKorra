...
public class PresetCommand extends PKCommand {
        ...
        // bending preset bind
            ...
            if (args.size() < 3) {
                ...
                if (target != null && target.isOnline()) {
                    ...
                    if (!boundAll) ChatUtil.sendBrandingMessage(target, ChatColor.RED + this.failedToBindAll);
                } else {
    ...
    protected List<String> getTabCompletion(final CommandSender sender, final List<String> args) {
        ...
        if (args.size() == 0) {
            ...
            l.add("bind");
            return l;
        } else if (args.size() <= 1 && (Arrays.asList("delete", "d", "del", "bind", "b").contains(args.get(0).toLowerCase()))) {
            ...
            if (sender.hasPermission("bending.command.preset.bind.external") && Arrays.asList("bind", "b").contains(args.get(0).toLowerCase())) {
                if (!Preset.externalPresets.isEmpty()) {
